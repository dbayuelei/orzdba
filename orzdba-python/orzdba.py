...
def get_options():
    ...
    parser.add_argument('-u', '--user', action='store',
                        help='user to use for mysql connection.')
...
def main():
    ...
    if gl.optflag == 0:
        sys.exit(0)
    gl.mysql_conn = 'mysql -s --skip-column-names -u%s -h%s -p%s -P%s ' % (gl.my_user, gl.my_host, gl.my_password, gl.my_port)
    if gl.my_socket is not None:
        gl.mysql_conn += ' -S' + gl.my_socket
    print (str(gl.mysql_conn))
    if gl.my_password is not None:
        shell_command = "env"
        os.putenv("MYSQL_PWD", gl.my_password)
