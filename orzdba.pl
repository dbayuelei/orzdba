...
my $interval        = 1;      # -i   : Time interval in seconds
my $load            = 0;      # -l   : Print load info
my $cpu             = 0;      # -c   : Print cpu  info
my $swap            = 0;      # -s   : Print swap info
my $disk;                     # -d   : Print disk info
my $mysql           = 0;      # -mysql : Print all of the MySQL status
my $com             = 0;      # -com : Print MySQL Com_xxx status
...
my $innodb_dblwrite = 0;      # -innodb_dbwrite : InnoDB Doublewrite Status
...
my $user;                     # -u   : MySQL's username
my $host;                     # -h   : MySQL's hostname
my $port            = 3306;   # -P   : MySQL's port number
my $passwd          = "";     # -p
my $socket;                   # -S
my $dbrt= 0;                  # -rt
my $tcprstat_dir    = "/tmp";
...
#
my $MYSQL    = qq{MYSQL_PWD=$passwd }  if( defined $passwd and $passwd ne "");
$MYSQL      .= qq{mysql -s --skip-column-names -P$port } if defined $user;
$MYSQL      .= qq{-u$user } if defined $user;
$MYSQL      .= qq{-h$host } if defined $host;
...
Command line options :
   ...
   -u,--user           User name of MySQL
   -h,--host           Host name of MySQL
   -P,--port           Port number of MySQL(default 3306)
   -p,--passwd         Password of MySQL(default null).
   -S,--socket         Socket file of MySQL
   ...
   -innodb_dblwrite     Print InnoDB Doublewrite Status(Innodb_os_log_fsyncs/written)
   ...
   -innodb_all         Print InnodbAllInfo(include -innodb,-innodb_dblwrite,-innodb_rows)
...
sub get_options{
  ...
  GetOptions(\%opt,
    ...
    'innodb_dblwrite',  # OUT : Print Innodb Double Status
    ...
    'innodb_all',       # OUT : Print Innodb Info
    ...
    $opt{'innodb_dblwrite'}   and $innodb_dblwrite = 1   and $mysql = 1;
    ...
    # -innodb_all
    $opt{'innodb_all'} and $timeFlag = 1 and $innodb_dblwrite = 1 and $innodb_rows=1 and $innodb_pages = 1 and $innodb_data = 1 and $innodb_log = 1 and $innodb_status = 1 and $mysql = 1;
    ...
    if($innodb_hit){
      $mysql_headline1 .= "-----bp read----- -Hit%- ";
      $mysql_headline2 .= "      lgr     phr   hit|";
    }
    if($innodb_rows){
    ...
    if($innodb_dblwrite){
      $mysql_headline1 .= "--double write-- ";
      $mysql_headline2 .= " writes written|";
    }
    if($innodb_log){
...
sub get_sysinfo{
  ...
  if($net) {
    open PROC_NET,"cat /proc/net/dev | grep '$net' | " or die "Can't open file(/proc/net/dev)!";
  ...
  if($disk) {
    ...
    open PROC_IO,"cat /proc/diskstats  | grep '$disk' |" or die "Can't open file(/proc/diskstats)!";
    if ( defined (my $line = <PROC_IO>) ) {
      ...
      my $r_await;       #/* read wait */
      my $w_await;       #/* write wait */
      ...
      $r_await   = $rd_ios ? $rd_ticks / $rd_ios : 0.0;
      $w_await   = $wr_ios ? $wr_ticks / $wr_ios : 0.0;
      ...
      #
      my $rkbs     = ($rd_sectors/$deltams /2) ;
      my $wkbs     = ($wr_sectors/$deltams /2) ;
      ...
      printf $LOG_OUT " %6.2f",$r_await and print $LOG_OUT RESET();
      $w_await>5  ? print $LOG_OUT RED() : print $LOG_OUT GREEN();
      printf $LOG_OUT " %6.2f",$w_await and print $LOG_OUT RESET();
      $wait>5  ? print $LOG_OUT RED() : print $LOG_OUT GREEN();
      printf $LOG_OUT " %6.2f",$wait and print $LOG_OUT RESET();
      $svc_t>5 ? print $LOG_OUT RED() : print $LOG_OUT WHITE();
      printf $LOG_OUT " %5.2f",$svc_t and print $LOG_OUT RESET();
      $busy>80 ? print $LOG_OUT RED() : print $LOG_OUT GREEN();
      printf $LOG_OUT " %6.2f",$busy and print $LOG_OUT RESET();
      print $LOG_OUT BLUE(),BOLD(),"|",RESET();
...
sub get_mysqlstat{
  if($mysql) {
    ...
    if ($not_first) {
      ...
      my $commit_diff               = ( $mystat2{"Com_commit"}           - $mystat1{"Com_commit"}           ) / $interval;
      my $rollback_diff             = ( $mystat2{"Com_rollback"}         - $mystat1{"Com_rollback"}         ) / $interval;

      my $Innodb_dblwr_written_diff = ( $mystat2{"Innodb_dblwr_pages_written"}  - $mystat1{"Innodb_dblwr_pages_written"}  ) / $interval;
      my $Innodb_dblwr_writes_diff  = ( $mystat2{"Innodb_dblwr_writes"}         - $mystat1{"Innodb_dblwr_writes"}         ) / $interval;
      ...
      if ($com) {
        ...
        # Total TPS
        printf $LOG_OUT " %5d",$commit_diff+$rollback_diff;
      ...
      if ($innodb_hit) {
        ...
        print $LOG_OUT WHITE();
        printf $LOG_OUT " %7d %7d",$read_request,$read;
     ...
     if ($innodb_status) {
       ...
       print $LOG_OUT WHITE();
       printf $LOG_OUT "%6d",$innodb_status{"history_list"};
       print $LOG_OUT GREEN(),"|",RESET();
       ...
       $innodb_status{"unflush_dirty_page"}/1024/1024 > 1 ?
       printf $LOG_OUT "%5.1fm ",($innodb_status{"unflush_dirty_page"}/1024/1024):
       printf $LOG_OUT "%6s ",($innodb_status{"unflush_dirty_page"}/1024 > 1 ? int($innodb_status{"unflush_dirty_page"}/1024 + 0.5)."k":$innodb_status{"unflush_dirty_page"});
       $innodb_status{"uncheckpointed_bytes"}/1024/1024 > 1 ?
       printf $LOG_OUT "%5.1fm",($innodb_status{"uncheckpointed_bytes"}/1024/1024):
       printf $LOG_OUT "%6s",($innodb_status{"uncheckpointed_bytes"}/1024 > 1 ? int($innodb_status{"uncheckpointed_bytes"}/1024 + 0.5)."k":$innodb_status{"uncheckpointed_bytes"});
       print $LOG_OUT GREEN(),"|",RESET();

       print $LOG_OUT WHITE();
       printf $LOG_OUT "%6d %6d",$innodb_status{"hash_sear"},$innodb_status{"non_hash_sear"};
       print $LOG_OUT GREEN(),"|",RESET();

       print $LOG_OUT WHITE();
       printf $LOG_OUT "%5d %6d %5d",$innodb_status{"read_views"},$innodb_status{"queries_inside"},$innodb_status{"queries_queue"};
       print $LOG_OUT GREEN(),"|",RESET();
     }
     ...
     if ($innodb_hit) {
       print $LOG_OUT WHITE();
       printf $LOG_OUT " %7d %7d %6.2f",0,0,100;
       print $LOG_OUT GREEN(),"|",RESET();
     }
...
sub get_innodb_status {
  ...
  foreach (@result) {
    ...
    # Pages flushed up to 6712508972870
    ...
    } elsif ( index($_,"Pages flushed up to") != -1 ) {
      my @tmp = split(/\s+/,$_);
      $innodb_status{"page_flushed"} = $tmp[4];
    ...
    # Hash table size 138401, node heap has 0 buffer(s)
    # Hash table size 138401, node heap has 365 buffer(s)
    # Hash table size 138401, node heap has 0 buffer(s)
    # Hash table size 138401, node heap has 0 buffer(s)
    # Hash table size 138401, node heap has 0 buffer(s)
    # Hash table size 138401, node heap has 1 buffer(s)
    # Hash table size 138401, node heap has 3 buffer(s)
    # 406000.00 hash searches/s, 241000.00 non-hash searches/s
    elsif ( index($_,"hash searches") != -1 ) {
      my @tmp = split(/\s+/,$_);
      $innodb_status{"hash_sear"}     = $tmp[0];
      $innodb_status{"non_hash_sear"} = $tmp[3];
    }
    #--------------
    # ROW OPERATIONS
    # --------------
    ...
    # 5.10 inserts/s, 29.38 updates/s, 0.02 deletes/s, 51322.87 reads/s
      ...
      $innodb_status{"queries_queue"}  = $tmp[4];
  ...
  $innodb_status{"unflush_dirty_page"}   = $innodb_status{"log_bytes_written"} - $innodb_status{"page_flushed"} ;
